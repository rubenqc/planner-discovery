...
export default {
  ...
  methods: {
    ...
    draw(site) {
      ...
      this.loadsites = true
      fetch(`${this.LINKS_URL}/getTophBySite?site=${site}&routers=true`)
      ...
      if(e.nodes.length > 0 && !this.onEdit){
        ...
        try{
          if(!node.isAllgraphic && node.loadtype === 'nearends'){
            ...
            const response = await this.$axios.post(`${this.LINKS_URL}/getDependecies?routers=true`,{
              nodes : this.nodes,
              edges : this.edges,
              site :  node.label
            })
            ...
            const responseFe = await this.$axios.post(`${this.LINKS_URL}/getFarEnds?routers=true`,{
              nodes : this.nodes,
              edges : this.edges,
              site :  node.label
            })
...
<style>
/*I should move this css into scc file*/
